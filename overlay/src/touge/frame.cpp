...
size_t encodePosition(const Position& p, uint8_t* out, size_t cap) {
  ...
  // and whether it is fit to keep time. All about the same claim, so they
  // travel together.
  out[17] = (uint8_t)((p.refHops & 0x0F) | (p.refLocked ? 0x10 : 0) | (p.refFit ? 0x20 : 0));
  out[18] = p.slot;
