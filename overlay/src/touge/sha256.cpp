...
void sha256(const uint8_t* data, size_t len, uint8_t out[SHA256_LEN]) {
  ...
  tail[rem++] = 0x80;
  // The length has to land in the last eight bytes. If the 0x80 pushed us past
  // where it fits, the padding spills into a second block.
  size_t pad = (rem <= 56) ? 64 : 128;
  memset(tail + rem, 0, pad - rem - 8);
