...
void OwnFix::fromPhone(const Fix& reading, uint32_t nowMs, uint32_t entropy) {
  ...
  if (!take(reading, nowMs, entropy)) return;
  fedMs_ = nowMs;
  phoneFedMs_ = nowMs;
  phoneFed_ = true;
}
...
bool OwnFix::take(const Fix& reading, uint32_t nowMs, uint32_t entropy) {
...
bool OwnFix::utcMs(uint32_t nowMs, uint64_t& out) const {
  if (!has_ || fix_.fixSec == 0) return false;
  out = (uint64_t)fix_.fixSec * 1000 + fix_.fixMs + (uint32_t)(nowMs - takenMs_);
  return true;
}
